...
def handlers_same_number():
        ...
        query = """
                SELECT
                    ...
                    h1.borrowings_approved
                    FROM handles h1
                    JOIN handles h2 ON h1.borrowings_approved = h2.borrowings_approved
                    JOIN user u1 ON h1.id = u1.id
                    JOIN user u2 ON h2.id = u2.id
                    WHERE u1.id <> u2.id
                    AND h1.borrowings_approved >20
