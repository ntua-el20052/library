...
-- Χρόνος δημιουργίας: 03 Ιουν 2023 στις 11:59:09
...
--
-- Δομή πίνακα για το view `active_borrowings_view`
--
CREATE VIEW active_borrowings_view AS
SELECT gt.name AS school_name, COUNT(*) AS active_borrowings_count
FROM user u
JOIN goes_to gt ON u.id = gt.id
JOIN borrowing br ON gt.id = br.id
WHERE br.status <> 'inactive'
AND br.borrowing_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 YEAR)
GROUP BY gt.name;  
