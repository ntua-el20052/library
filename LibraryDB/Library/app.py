...
app.config['MYSQL_DB'] = 'library_github_new_5'
...
class ChangeApprovalStatusForm(FlaskForm):
    name = StringField('Name', validators=[DataRequired()])
    new_status = StringField('New Approval Status', validators=[DataRequired()])
...
def change_approval_status():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        cursor = mysql.connection.cursor()
        query = "UPDATE user SET approved_status = %s WHERE name = %s"
        cursor.execute(query, (new_status, name))
        mysql.connection.commit()
        ...
        return "Your card:     Name=" + name + ",     ID=" + str(id) +",    Authority=" + str(auth)
...
def add_new_book():
    ...
    if request.method == 'POST' and form.validate_on_submit():
             ...
             cursor = mysql.connection.cursor()
             query = "INSERT INTO books(isbn, title, publisher, author,  pages, summary, nmbr_of_copies,image,  genre, language, key_words) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
             cursor.execute(query, (isbn, title,publisher,author,pages,summary,nmbr_of_copies,image, genre,language,key_words))
             mysql.connection.commit()
...
def edit_book_information():
    ...
    if request.method == 'POST' and form.validate_on_submit():
             ...
             cursor = mysql.connection.cursor()
             query = "UPDATE books SET title = %s, publisher = %s, author = %s, pages = %s, summary= %s, nmbr_of_copies = %s, image = %s, genre = %s, language = %s, key_words = %s  WHERE isbn = %s"
...
#Access bookings table; for school Operator
...
#gia ta reservetion, an diagrafei kapoio reservation na ginei borrowing
@app.route('/backup', methods=['GET'])
def backup():
    subprocess.run(['mysqldump', '-u', 'root', '-p', '', 'library_github_new_5', '>', 'backup.sql'], shell=True)
    return 'Database backup completed'


@app.route('/restore', methods=['POST'])
def restore():
    # Get the backup file from the request form
    backup_file = request.files['backup_file']

    # Save the backup file to a local directory
    backup_file.save('backup.sql')

    subprocess.run(['mysql', '-u', 'root', '-p', '', 'library_github_new_5', '<', 'backup.sql'], shell=True)
