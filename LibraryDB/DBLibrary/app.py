...
def add_new_school():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        query = "INSERT INTO manages(name, id) VALUES(%s, 799)"
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        query = """
            SELECT user.*
              ...
              WHERE user.id = %s AND DATE_ADD(borrowing.borrowing_date, INTERVAL 7 DAY) < NOW()
          """
        cursor.execute(query, (id,))
...
def search_for_delays_days():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        query = """
            ...
            FROM user
            JOIN borrowing ON user.id = borrowing.id
            WHERE DATE_ADD(borrowing.borrowing_date, INTERVAL 7 DAY) < DATE_ADD(NOW(), INTERVAL %s DAY)
        """
...
def teachers_borrowed_category():
        ...
        query = """
                  SELECT * FROM young_teachers_most_books;
                """
        cursor.execute(query)
...
def handlers_same_number():
        ...
        query = """
                  SELECT h.handle_id, h.school_name
                        FROM handles h
                        JOIN goes_to gt ON h.id = gt.id
                        JOIN goes_to gtp ON h.school_name = gtp.name
                        JOIN borrowing bo ON gtp.id = bo.id
                        WHERE YEAR(bo.borrowing_date) = YEAR(CURRENT_DATE)
                        GROUP BY h.handle_id, h.school_name
                        HAVING COUNT(*) > 20
                """
        cursor.execute(query)
...
#reviews text sto review_book kai sto read_review
