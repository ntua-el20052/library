...
-- Χρόνος δημιουργίας: 26 Μάη 2023 στις 16:44:29
...
DROP SCHEMA IF EXISTS library_github_new_3;
CREATE SCHEMA library_github_new_3;
USE library_github_new_3;
...
CREATE TRIGGER `check_booking_limit_trigger` BEFORE INSERT ON `booking` FOR EACH ROW BEGIN
    ...
    SELECT number, nmbr_of_books INTO role, borrow_count
    FROM user
    WHERE id = NEW.id;
...
('1111111111111', 'The Great Gatsby', 'Scribner', 'F. Scott Fitzgerald', 218, 'hello', 8, '[Image URL or file path]', 'Fiction', 'English', 'Jazz Age, American Dream, wealth, love, decadence'),
('2222222222222', 'programming in unix', 'kleidarithmos', 'marc j rochkind', 768, 'programming on unix', 3, 'link', 'math,programming', 'greek', 'math,unix,programming');
...
(82, 52, '1111111111111', '2023-05-18', 'returned'),
(86, 52, '0000000000000', '2023-05-26', 'delayed');
...
CREATE TRIGGER `insert_borrowing_trigger` BEFORE INSERT ON `borrowing` FOR EACH ROW BEGIN
    ...
    SET bor_books = (SELECT COUNT(*) FROM borrowing WHERE id = NEW.id AND (status = 'active' OR status = 'delayed'));
...
INSERT INTO `user` (`id`, `password`, `name`, `age`, `ph_nmbr`, `mail`, `nmbr_of_books`, `number`, `approved_status`) VALUES
(52, '', 'pavlos skou', 45, '6947177115', 'pavlos.skou@gmail.com', 2, 1, NULL),
...
(982, '', 'ioanna siakka', 28, '6954321465', 'ioannasiakka@gmail.com', 0, 2, NULL);
...
--
ALTER TABLE `borrowing`
  MODIFY `borrowing_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=87;
